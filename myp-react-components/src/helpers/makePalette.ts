...
import PaletteColor from '../interfaces/PaletteColor'
...
export function makePalette({ primary, secondary, feedback = {}, neutral = '#ADB5BD' }: MakePallete): Theme {
...
export function MakePaletteColor(color: string): PaletteColor {
  ...
  return {
    darkest: parsedColor.darken(0.4).hex().toString(),
    dark: parsedColor.darken(0.2).hex().toString(),
